...
####################################################################################################
# Resource-estimation exercises: swap the adder to hit an active-volume (and qubit) budget.
# Graded interactively from the notebook via the `problem` decorator, not by pytest.

AV_THRESHOLD_1 = 260_000
AV_THRESHOLD_2 = 280_000
HW_THRESHOLD_2 = 26


@mark.parametrize("build_fn", [ref.exercise_1] if ref_available else [])
def test_exercise_1(build_fn):
    av = resource_estimator(build_fn()).resources()["active_volume"]
    assert av < AV_THRESHOLD_1, f"active volume {av:,.0f} is not below {AV_THRESHOLD_1:,}"
    print(f"Active volume: {av} is below {AV_THRESHOLD_1}!")


@mark.parametrize("build_fn", [ref.exercise_2] if ref_available else [])
def test_exercise_2(build_fn):
    resources = resource_estimator(build_fn()).resources()
    av, highwater = resources["active_volume"], resources["qubit_highwater"]
    assert av < AV_THRESHOLD_2, f"active volume {av:,.0f} is not below {AV_THRESHOLD_2:,}"
    assert highwater <= HW_THRESHOLD_2, f"qubit highwater {highwater} exceeds {HW_THRESHOLD_2}"
    print(f"Active volume: {av} is below {AV_THRESHOLD_2}!")
    print(f"Qubit highwater: {highwater} is less or equal to {HW_THRESHOLD_2}!")


####################################################################################################
# Mocking-qubricks exercise: a QubrickCosts-based mock whose active_volume scales as N**2 and whose
# qubit_highwater (from real ancilla allocation) scales as ceil(log2 N). Graded from the notebook.


@mark.parametrize("qbk_class", [ref.AbstractRoutine] if ref_available else [])
def test_abstractroutine(qbk_class):
    for N in [8, 64, 1024]:
        global log_message
        log_message = f"Testing N={N}"

        qpu = QPU(num_qubits=64, filters=[">>buffer>>"])
        qbk_class(N=N, qc=qpu).compute()  # no register args -> pass qc= explicitly
        resources = resource_estimator(qpu).resources()

        expected_av = N ** 2
        if resources["active_volume"] != expected_av:
            raise Exception(
                f"active_volume should scale as N**2: expected {expected_av}, "
        ...
        if resources["qubit_highwater"] != expected_highwater:
            raise Exception(
                f"qubit_highwater should scale as ceil(log2 N): expected {expected_highwater}, "
